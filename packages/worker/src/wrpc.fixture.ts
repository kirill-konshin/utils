...
/*
 * isolatedDeclarations needs an explicit type on any exported value initialized via a generic
...
const responders = {
    ...
    setTimeout: async ({ timeout = 1000 }: { timeout?: number } = {}): Promise<unknown> =>
        new Promise((resolve) => setTimeout(resolve, timeout)),

    setInterval: async function* ({ timeout = 1000 }: { timeout?: number } = {}): AsyncGenerator<
        unknown,
        never,
        unknown
    > {
