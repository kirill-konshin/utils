...
3. Library packages run `start` (vite build --watch) which produces `dist/index.js`
...
1. **Task inheritance**: Tasks `build`, `clean`, `test`, `start`, `wait` are defined in root `turbo.json` and inherited by all packages
...
4. **Check `outputs`**: Must include all build artifacts (`dist/**/*`, `package.json` if modified, `src/**/*/index.ts` if generated)
